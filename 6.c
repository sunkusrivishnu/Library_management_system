...
//function for converting the string into all lowercase letters
...
void stringlower2(char *s1)
{
    for (int j = 0;j < strlen(s1) ;j++)
	{
		if (s1[j] >= 'A' && s1[j] <= 'Z')
		{
			s1[j] = s1[j] + 32;		//converting into lowercase.
		}
	}
}
...
int stringcompare2(char *s1, char *s2)
{
    if (strlen(s1) <= strlen(s2))
    {
        ...
        return 0;		// returns 0 if matched.
    }
    return 1;
}

//fucntion to search issue records by issuername.
...
void search_by_issuername(int ni, issuer issue_rec[])
{
    ...
    char issuername[140];
	scanf("%[^\n]%*c",issuername);			//taking input of issuer's name.
    stringlower2(issuername);				//converting all to lowercase for convenience.
	...
	for(int i = 0 ; i < ni ;i++)
	{
        ...
        if (stringcompare2(issuername,issue_rec[i].iname) == 0 )		//if the input matches the issuername in the specific record, printing the record.
        {
    ...
    if(count==0)
    {
        printf("NO DATA FOUND\n");		//if none of the records contain the issuername input.
    }
